...
- [x] 工具调用内部完成提交→轮询→返回最终结果，客户端无感异步
- [x] 轮询请求绑定提交时所用的上游密钥，映射持久化可重启恢复
- [x] 总超时 10 分钟，超时返回明确错误
- [x] research 任务失败（上游 failed 状态）原样反馈给调用方
- [x] 测试：mock 上游模拟 pending→completed 全流程；绑定 key 断言；超时路径
